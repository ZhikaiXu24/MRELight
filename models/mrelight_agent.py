...
The agent uses multi-objective Q-values for traffic signal control.
The two reward objectives are queue length and efficient average speed.
Action selection is performed through Tchebycheff scalarization.
"""
import re
import numpy as np
import os
...
from utils.scalarization import tchebicheff
from utils.scalarization import select_best_action

_scalarization_cache = None
def build_memory():
    return []


class MRELightAgent(Agent):
    def __init__(self, dic_agent_conf=None, dic_traffic_env_conf=None, dic_path=None, cnt_round=None,
                 intersection_id="0"):
        global _scalarization_cache
        super(MRELightAgent, self).__init__(
            dic_agent_conf, dic_traffic_env_conf, dic_path, intersection_id)
        if _scalarization_cache is None:
            tau = self.dic_agent_conf.get("TCHEBICHEFF_TAU", 4)
            _scalarization_cache = tchebicheff(tau=tau, reward_dim=2)
        self.scalarization = _scalarization_cache
        ...
        self.memory = build_memory()#创建了一个空列表
        ...
        if cnt_round == 0:
            ...
            try:
                self.load_network("round_{0}_inter_{1}".format(cnt_round - 1, self.intersection_id))#加载已有的模型参数
    ...
    def choose_action(self, count, states):
        ...
        q_values = self.q_network(xs)[0]

        if random.random() <= self.dic_agent_conf["EPSILON"]:
            action = np.random.randint(self.num_actions, size=q_values.shape[0])
        else:
            weights = np.array(
                self.dic_agent_conf.get("OBJECTIVE_WEIGHTS", [0.7, 0.3]),
                dtype=float
            )
            tche_func = self.scalarization
            actions = []
            for agent_i in range(q_values.shape[0]):
                best_action = select_best_action(q_values[agent_i], weights, tche_func)
    ...
    def prepare_Xs_Y(self, memory):
        ...
        _reward = [[] for _ in range(self.num_agents)]# multi-objective: [queue_length, avg_speed]
        ...
        for i in range(slice_size):
            ...
            for j in range(self.num_agents):
                ...
                state, action, next_state, reward_1, reward_2, *_ = memory[j][i]
                _action[j].append(action)
                _reward[j].append([reward_1,reward_2])
        ...
        #q网络计算输出
        q_values = self.q_network([_state2, _adjs2])
        next_q_values = self.q_network_bar([_next_state2, _adjs2])
        # [batch, agent, num_actions]
        final_target = np.copy(q_values)
        #tchebicheff
        weights = np.array(
            self.dic_agent_conf.get("OBJECTIVE_WEIGHTS", [0.7, 0.3]),
            dtype=float
        ...
        for i in range(slice_size):
            for j in range(self.num_agents):
                ...
                action_qs = next_q_values[i, j, :, :]
                best_a = select_best_action(action_qs, weights, tche_func)
                
                next_q_max = action_qs[best_a,:]
                final_target[i, j, _action[j][i],:] = (
                    _reward[j, i, :] / self.dic_agent_conf["NORMAL_FACTOR"]
                    + self.dic_agent_conf["GAMMA"] * next_q_max
                )
    ...
    def build_network(self, MLP_layers=[32, 32]):
        ...
        CNN_heads = [5] * len(CNN_layers)   #五头注意力
        In = list()
        # In: [batch,agent,dim]
        # In: [batch,agent,neighbors,agents]
        # 明确告诉模型输入数据的结构特征，因此也说明了为什么要计算lenth
        # 状态特征输入
        ...
        # [batch,agent,32]->[batch,agent,action*2(8)]
        out = Dense(self.num_actions*2, kernel_initializer='random_normal', name='action_layer')(h)
        out = Reshape((self.num_agents, self.num_actions, 2))(out)
