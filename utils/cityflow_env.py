...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        # Store the first time and distance when each vehicle enters the last 150 meters.
        self.enter_150m_time_dict = {}
        self.enter_150m_distance_dict = {}
    ...
    def set_signal(self, action, action_pattern, yellow_time, path_to_log):
        if self.all_yellow_flag:
            ...
            # determine phase    mode=switch:0保持，1下一个：按照固定的顺序依次切换相位
            ...
            #set模式，任意选择，由于索引从0开始，而相位是1-4，所以需要+1
            ...
            # set phase当交通信号灯需要切换相位时，先切换到黄灯状态，并记录相关日志和标志位。
    ...
    def update_current_measurements(self, simulator_state):
        ...
        # can't use empty set to - real set哪些车辆在上一个时间步存在，但在当前时间步已经离开了进入车道
    ...
    def _update_feature(self):
        ...
        # 将进入和离开车道的等待车辆数量进行整理且分为三个部分，以适应多阶段交通流量描述。
        ...
        dic_feature["average_vehicle_speed"] = self._get_average_vehicle_speed()
    ...
    def _get_traffic_movement_pressure_general(enterings, exitings):
        ...
        # 对于每个车道（索引 j），计算其进入车辆数量 enterings[j] 减去对应转向方向的出口车道车辆总数
    ...
    def _get_part_traffic_movement_features(self):
        ...
        """
        # 分别表示每个车道前部的车辆数量、后部的车辆数量和后部的等待车辆数量。
      ...
      #    分别表示  基于总车辆数量的交通压力。基于等待车辆数量的一般交通压力。基于等待车辆数量的高效交通压力。进入车道后部的运行车辆数量。进入车道后部的等待车辆数量。
    ...
    @staticmethod
    #用于获取车道上车辆的部分观察信息。该函数将每个车道上的车辆分为两部分：前部和后部，并分别统计这两部分的车辆数量和等待车辆数量。
    def _get_part_observations(lane_vehicles, vehicle_distance, vehicle_speed,
        ...
        # 返回三个字典，分别表示每个车道前部的车辆数量、后部的车辆数量和后部的等待车辆数量。
        return first_part_num_vehicle, last_part_num_vehicle, last_part_queue_vehicle
   
    def _get_average_vehicle_speed(self):
        current_time = self.get_current_time()
        # 为每条入口车道建立速度和车辆计数器
        lane_speed_sum = {lane: 0.0 for lane in self.list_entering_lanes}
        lane_vehicle_cnt = {lane: 0   for lane in self.list_entering_lanes}

        for lane in self.list_entering_lanes:
            lane_length = self.lane_length[lane] #获取当前车道长度
            for vehicle in self.dic_lane_vehicle_current_step[lane]:
                if vehicle not in self.dic_vehicle_arrive_leave_time:
                    continue

                record = self.dic_vehicle_arrive_leave_time[vehicle]
                distance = self.dic_vehicle_distance_current_step.get(vehicle)
                if distance is not None:
                    remaining_distance = lane_length - distance
                    # 进入150米内时记录times
                    if remaining_distance < 150:
                        if vehicle not in self.enter_150m_time_dict:
                            self.enter_150m_time_dict[vehicle] = current_time
                            self.enter_150m_distance_dict[vehicle] = distance

                    # 只统计还没离开的150米区间车辆
                    if vehicle in self.enter_150m_time_dict and np.isnan(record["leave_time"]):
                        enter_150m_time = self.enter_150m_time_dict[vehicle]
                        enter_150m_distance = self.enter_150m_distance_dict[vehicle]
                        time_in_150m_range = current_time - enter_150m_time                        
                        distance_in_150m_range = distance - enter_150m_distance 
                        if time_in_150m_range > 0:
                            speed = distance_in_150m_range / time_in_150m_range
                            lane_speed_sum[lane] += speed
                            lane_vehicle_cnt[lane] += 1

        # 每个lane输出平均速度
        average_lane_speed = []
        for lane in self.list_entering_lanes:
            if lane_vehicle_cnt[lane] > 0:
                avg_speed = 3.6 * lane_speed_sum[lane] / lane_vehicle_cnt[lane]
            else:
                avg_speed = 0 # 如果没有车辆，则设为0km/h
            # 取整
            average_lane_speed.append(round(avg_speed))
        return average_lane_speed
    ...
    def get_reward(self, dic_reward_info):
        ...
        dic_reward["average_speed"] = round(np.mean(self.dic_feature["average_vehicle_speed"]))
        dic_reward["pressure"] = np.absolute(np.sum(self.dic_feature["pressure"]))

        reward = 0
        for r in dic_reward_info:
            if dic_reward_info[r] != 0:
                if r not in dic_reward:
                    raise KeyError("Reward key '{}' is not supported. Supported keys are {}.".format(
                        r, list(dic_reward.keys())
                    ))
...
class CityFlowEnv:
    ...
    def reset(self):
        ...
        # get adjacency当前信号灯位置，所有信号灯编号，临近的四个（5头），ENWS
        self.traffic_light_node_dict = self._adjacency_extraction()

        # get lane length道路实际长度，不要归一化长度（除以400）
        _, self.lane_length = self.get_lane_length()

        # 将刚才得到的信号灯信息和道路长度等转换为列表list_intersection；initialize intersections (grid)
        ...
        for i in range(self.dic_traffic_env_conf["NUM_COL"]):
            for j in range(self.dic_traffic_env_conf["NUM_ROW"]):
                ...
                count += 1
        #将所有交通路口的车道信息合并到一个列表中。
        self.list_lanes = []
        for inter in self.list_intersection:
            self.list_lanes += inter.list_lanes
        self.list_lanes = np.unique(self.list_lanes).tolist()#去除重复的车道信息。并将结果从 NumPy 数组转换回 Python 列表

        # 这些状态信息将被用于更新每个交通路口的内部状态
    ...
    def step(self, action):
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]-1):
            if self.dic_traffic_env_conf["ACTION_PATTERN"] == "switch":#按照固定相位顺序转变
                list_action_in_sec.append(np.zeros_like(action).tolist())
            elif self.dic_traffic_env_conf["ACTION_PATTERN"] == "set":#直接设定相位
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            for j in range(len(reward)):#计算每个十字路口的一个time_step的平均奖励
    ...
    def get_reward(self):#每个十字路口都有一个复合reward
    ...
    def bulk_log_multi_process(self, batch_size=100):
        ...
        for t in process_list:
            t.join()
        print("end join")
    #从 CityFlow 的路网文件中提取交通灯节点的邻接关系和相关信息。
    def _adjacency_extraction(self):
        ...
        with open("{0}".format(file)) as json_data:
            ...
            # 遍历所有的十字路口，为每个非虚拟节点创建一个字典条目，记录其位置（x, y）。
            ...
            for i in traffic_light_node_dict.keys():
                ...
                index += 1
            #最短距离法，遍历所有的交点，并计算出每个交点与其他交点的距离（最短距离法），找到最近的五个邻居，并将结果存储在adjacency_row中。
            for i in traffic_light_node_dict.keys():
                ...
                elif len(row) > top_k:#找到距离最近的 top_k 个元素的索引
                    adjacency_row_unsorted = np.argpartition(row, top_k)[:top_k].tolist()
                else:
                    adjacency_row_unsorted = [k for k in range(total_inter_num)]
                adjacency_row_unsorted.remove(inter_id_to_index[i])#去除自己对自己
                traffic_light_node_dict[i]["adjacency_row"] = [inter_id_to_index[i]]+adjacency_row_unsorted#第一个元素是节点 i 自己的索引，后续元素是按距离排序的最近邻居节点的索引。
                traffic_light_node_dict[i]["total_inter_num"] = total_inter_num
             #为每个交通灯节点（交叉口）识别其在东、南、西、北四个方向上的邻接交叉口，并将这些邻接交叉口的信息存储在 neighbor_ENWS 列表中。
            for i in traffic_light_node_dict.keys():
        ...
        return traffic_light_node_dict#当前信号灯位置，所有信号灯编号，临近的四个（5头），ENWS
    #计算两个交叉路口之间的欧几里得距离（根号下平方和）
    @staticmethod
    ...
    #从路网文件中提取车道长度信息，并对其进行归一化处理(分别除以最短长度)。
    def get_lane_length(self):
        ...
        min_length = min(lanes_length_dict.values())#用于归一化
